    ...
    if (fw == -1) {
        ...
        if (ask=="") {api.sendMessage("Thiếu câu hỏi kìa;-;",threadID,messageID)} else {
            if (!ans) {api.sendMessage("Thiếu câu trả lời kìa;-;",threadID,messageID)} else {
                    axios.get(encodeURI(`https://sim.botviet.me/daysim?ask=${ask}&ans=${ans}`)).then(res => {
                        if (res.data.answer == "Câu hỏi với Câu trả lởi có hết cmnr, thêm cái cc;-;"){
                            api.sendMessage("Câu hỏi, Trả lời này có rồi ;-;",threadID,messageID)} else {
                                if (res.data.answer == "Bị lỗi cc gì đó đéo biết nữa") {api.sendMessage('Lỗi không xác dịnh;-;',threadID,messageID)} else {
                                    api.sendMessage("Em đã tiếp thu bài học!",threadID,messageID);
                                }
                            }
                    })
            }
